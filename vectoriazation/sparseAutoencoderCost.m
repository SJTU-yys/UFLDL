...
st1 = theta.^2;
J_weight = (lambda/(2*m))*sum(st1(:));
J_sparse = sum(sparsityParam.*log(sparsityParam./rho)+(1-sparsityParam).*log(1-sparsityParam)./(1-rho));

%total cost
cost = J_cost+(lambda/2)*J_weight+beta*J_sparse;

% calculate the Delta 
delta3 = -(data - a3).*sigmoid(z3).*(1-sigmoid(z3)); % No relative entrophy
sterm = beta*(-sparsityParam./rho+(1-sparsityParam)./(1-rho));
delta2 = (W2'*delta3 + beta*repmat(sterm,1,m)).*sigmoid(z2).*(1-sigmoid(z2));
